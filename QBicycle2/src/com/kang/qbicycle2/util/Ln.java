 ...
 * roboguice/src/main/java/roboguice/util/Ln.java
 * 
 * A more natural android logging facility.
 * 
 * WARNING: CHECK OUT COMMON PITFALLS BELOW
 * 
 * Unlike {@link android.util.Log}, Log provides sensible defaults. Debug and
 * Verbose logging is enabled for applications that have
 ...
 * r8 or later will have verbose and debug log messages turned off.
 * 
 * The default tag is automatically set to your app's packagename, and the
 * current context (eg. activity, service, application, etc) is appended as
 * well. You can add an additional parameter to the tag using
 * {@link #Log(String)}.
 * 
 * Log-levels can be programatically overridden for specific instances using
 * {@link #Log(String, boolean, boolean)}.
 * 
 ...
 * Additional parameters to the logging statement are treated as varrgs
 * parameters to {@link String#format(String, Object...)}
 * 
 * Also, the current file and line is automatically appended to the tag (this is
 * only done if debug is enabled for performance reasons).
 * 
 * COMMON PITFALLS: * Make sure you put the exception FIRST in the call. A
 * common mistake is to place it last as is the android.util.Log convention, but
 * then it will get treated as varargs parameter. * vararg parameters are not
 * appended to the log message! You must insert them into the log message using
 * %s or another similar format parameter
 * 
 * Usage Examples:
 * 
 * Ln.v("hello there"); Ln.d("%s %s", "hello", "there"); Ln.e( exception,
 * "Error during some operation"); Ln.w( exception, "Error during %s operation",
 * "some other");
 * 
 * 
 */
